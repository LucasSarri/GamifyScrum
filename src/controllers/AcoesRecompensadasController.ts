...
export class AcoesRecompensadasController {
    ...
    async addAcoesRecompensadasPlanejamento (req: Request, res: Response) 
    {
        ...
        const acoesrecompensadasExistentes = await planejamento.acoes_recompensadas;

        acoesrecompensadas.forEach((acaorecompensada) => {
            if (!acoesrecompensadasExistentes.some((c) => c.id === acaorecompensada.id)) {
                acoesrecompensadasExistentes.push(acaorecompensada);
            }
        });
